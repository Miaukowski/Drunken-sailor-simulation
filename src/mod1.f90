...
module mtdefs
  ...
  integer :: ymax=0  ! Just for debug purposes.
end module mtdefs
...
! Modified grnd() so that it does not return 1.0, A.Kuronen, 2022 !
!                                                                 !
!-----------------------------------------------------------------!
!
...
module mtmod
  ...
  use mtdefs, only : rk_mtmod => rk,ymax  ! ymax for debug purposes
  implicit none
...
contains
  ...
  ! [18.2.2022] Added a dirty hack to prevent getting grnd()==1   !
  ! [6.3.2022]  Dirty hack is no more necessary: In converting    !
  !             from integer to double, increased denominator by  !
  !             one: 2³¹-1 --> 2³¹ (see below).                   !
  !                                                               !
  !---------------------------------------------------------------!
  ...
  function grnd()
    ...
    if (y>ymax) ymax=y ! Just for debug purposes.
    
    ! Converting from integer to double
    if (y<0) then
       ! Had to increase the denominator by
       ! one in order to prevent return of 1.0
       !grnd0=(dble(y)+2.0d0**32)/(2.0d0**32-1.0d0)
       grnd=(dble(y)+2.0d0**32)/(2.0d0**32)
    else
       !grnd0=dble(y)/(2.0d0**32-1.0d0)
